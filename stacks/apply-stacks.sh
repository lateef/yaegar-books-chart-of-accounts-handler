...
jq -S '.Stacks[0].Outputs[] | select(.OutputKey=="TableNameMasterList").OutputValue')

table_name_industry=$(aws cloudformation describe-stacks --stack-name ${stack_name} |
jq -S '.Stacks[0].Outputs[] | select(.OutputKey=="TableNameByIndustry").OutputValue')

echo ${config_content} | jq 'setpath(["environment"]; '\"${lowerEnv}\"')' | jq 'setpath(["dynamodb", "yaegarBooksChartOfAccountsTable"]; '"${table_name}"')' | jq 'setpath(["dynamodb", "chartOfAccountsByIndustryTable"]; '"${table_name_industry}"')' > ${config_file}
